---
title: "Cash-out flow"
description: "End-to-end sequence for moving USDC from an Archer wallet to a fiat bank account through Bridge KYC, liquidation addresses, and off-ramp settlement."
sidebarTitle: "Cash-out flow"
---

Cash-out is the composition of three subsystems: Bridge (KYC \+ off-ramp), the Archer wallet (USDC transfer), and the host wallet-authorization surface (signature).

## Actors

- **End user** — owns the wallet, completes KYC, taps “Cash out”.
...
4. **Prepare.** `prepareCashOut` builds an `erc20_transfer` intent from the end user's wallet to the liquidation address and asks Archer to prepare the ERC-4337 user operation. If the wallet is not yet [EIP-7702 delegated](/concepts/wallet-authorization), the response also includes a `delegation` block that must be signed alongside the user operation.
5. **Authorize.** The host calls its wallet-authorization surface with `prepared.intent` and `prepared.prepared.request_payload` (plus `prepared.prepared.delegation.request_payload` if present), receives signatures, and packages them as `SignedPayload`.
...
- If step 6 succeeds but the user operation reverts on chain, the response carries a `tx_hash`; the funds never leave the wallet.
